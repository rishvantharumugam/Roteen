...
async function fetchSubjectRow(filter: VideoSubjectFilter) {
  ...
  const fuzzyMatch = fuzzyRows?.find((row) => standardMatches(row.standard, effectiveStandard)) ?? fuzzyRows?.[0] ?? null;
...
export async function fetchSubjectPanelData(
  ...
  const fetchPromise = (async () => {
    ...
    const existingChapterIds = new Set((chapterRows ?? []).map((chapter) => String(chapter.id)));
