  ...
  return (
    <main className={`bg-black text-zinc-200 min-h-screen video-smooth dark box-border w-full max-w-[100vw] min-h-screen overflow-hidden  text-zinc-100 box-border flex h-screen flex-col overflow-hidden`}>
      ...
      <motion.div className="box-border flex flex-col lg:flex-row flex-1 min-w-0 overflow-hidden px-4 pb-4 pt-4">
        ...
        <div
          className={`transition-all duration-300 ease-in-out flex shrink-0 
            ${theoryViewEnabled
              ...
              : `w-full lg:w-[20%] opacity-100 translate-x-0 lg:pr-4 ${activeMobileTab === "index" ? "flex" : "hidden lg:flex"}`
            }`}
        >
        ...
        {isChapterQuizView ? (
          <div
            className={`transition-all duration-300 ease-in-out flex shrink-0 ${theoryViewEnabled
                ? "w-0 opacity-0 -translate-x-8 overflow-hidden pointer-events-none"
          ...
          <>
            ...
            ${theoryViewEnabled
                  ? "w-0 opacity-0 -translate-x-8 overflow-hidden pointer-events-none"
                  : `w-full lg:w-[40%] opacity-100 translate-x-0 lg:pr-4 ${activeMobileTab === "video" ? "flex" : "hidden lg:flex"}`
                }`}
            >
            ...
            ${theoryViewEnabled
                  ? "w-full"
                  : `w-full lg:w-[40%] ${activeMobileTab === "theory" ? "flex" : "hidden lg:flex"}`
                }`}
            >
              <aside
                className={`rounded-2xl border border-zinc-800/90 bg-[#161616] shadow-[0_18px_36px_rgba(0,0,0,0.42)] animate-fade-in-right box-border h-full w-full min-w-0 flex shrink flex-col overflow-hidden transition-all duration-300 ${theoryViewEnabled
                    ? "border-purple-500/40 shadow-[0_0_40px_rgba(168,85,247,0.15)] brightness-105"
                    : ""
                  }`}
                style={{ animationDelay: "160ms" }}
              >
                <div className="flex w-full shrink-0 items-center justify-between border-b border-zinc-800 p-4 overflow-x-auto no-scrollbar flex-nowrap gap-4">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 shrink-0">
                        ...
                        {(["English", "Tamil"] as const).map((lang) => {
                          ...
                          return (
                            <motion.button
                              ...
                              className={`relative flex flex-1 items-center justify-center rounded-full px-4 py-1 text-[11px] font-semibold transition-colors duration-300 ${isActive ? "text-white" : "text-zinc-500 hover:text-zinc-300"
                                }`}
                    ...
                    <button
                      ...
                      className="hidden md:flex items-center gap-2 whitespace-nowrap text-sm text-zinc-300 transition-all duration-300"
