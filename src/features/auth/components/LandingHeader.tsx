    ...
    <span className="relative inline-flex items-end font-heading text-[1.72rem] font-black italic leading-none tracking-normal sm:text-[1.9rem]">
      ...
      <span className="relative text-violet-400">
        <span className="absolute -top-3 left-1/2 hidden h-4 w-9 -translate-x-1/2 sm:block">
          <span className="absolute left-1 top-2 h-2.5 w-4 -rotate-12 rounded-t-full border-t-2 border-white" />
          <span className="absolute right-1 top-2 h-2.5 w-4 rotate-12 rounded-t-full border-t-2 border-white" />
        </span>
        een
      </span>
    </span>
  ...
  return (
    <header
      className={`landing-header sticky top-0 z-40 px-4 transition-all duration-300 sm:px-6 lg:px-8 ${
        isScrolled ? "py-2 premium-glass-nav" : "py-3 bg-transparent"
      }`}
    >
      <div
      ...
      >
        <Link href={appRoutes.home} className="flex items-center text-white premium-button-hover">
          <RoteenWordmark />
        </Link>
